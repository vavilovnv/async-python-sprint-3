...
Для запуска серверной части нужно запустить модуль server.py. Для запуска клиента - модуль client.py. Можно запустить любое количество клиентов. Соединение клиента с сервером происходит автоматически.


## Команды взаимодействия с сервером:

Команды набираются в консоли. Параметры команд обозначены угловыми скобками `<...>`. Опциональные команды обозначены квадратными скобками `[...]`.
```
...
/status - показать статус пользователя: адреса соединений, количество приватных сообщения, администрирование приватных чатов, участие в приватных чатах и инвайт-ключи к ним
...
/join <chat name> [invite-key] - запрос на получение инвайта для присоединения к чату (команда без параметра invite-key) или присоединение к приватному чату при имеющемся инвайт-ключе
```
